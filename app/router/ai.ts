...
export const generateThreadSummary=base
  ...
  .handler(async({input,context,errors})=>{
    ...
    const system=[
        "You are an expert assistant summarizing Slack-line discussion threads for a product team.",
        ...
        "- Then add a blank line followed by exactly 2-3 bullet points (using '-') with the most important takeways. Each bullet is one sentence.",
        "Style: neutral, specific, and concise. Preserve terminology from the thared (names,acronyms). Avoid filler or meta-commentary. Do not add a closing sentence.",
        "If the context is insufficient, return a single-sentence summart and omit the bullet list.",
    ].join("\n");
...
export const generateCompose=base
 ...
 .handler(async({input})=>{
    ...
    //Focused system prompt for deterministic rewrite (non-controversial)
    const system=[
        "You are an expert rewriting assistant. You are not a chatbot.",
        "Task: Rewrite the provided content to be clearer and better structuredwhile preserving meaning, facts, terminology, and names.",
    ...
    const result=streamText({
        ...
        messages: [
            {
                role:"user",
                content:'Please write and improve the following content:'
            },
            {
                role:'user',
                content: markdown,
