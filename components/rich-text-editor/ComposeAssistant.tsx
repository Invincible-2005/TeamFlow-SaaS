...
export function ComposeAssistant({content,onAccept}:ComposeAssistant){
    ...
    const composedText=lastAssistant?.parts.filter((p)=>p.type==='text').map((p)=>p.text).join('\n\n') ?? "";
    ...
    return(
        <Popover open={open} onOpenChange={handleOpenChange}>
            <PopoverTrigger asChild>
                <Button type="button" size="sm" className="relative overflow-hidden rounded-full bg-linear-to-t from-violet-600 to-fuchsia-600 text-white shadoew-md hover:shadow-lg focus-visible:ring-2 focus-visible:ring-ring">
            ...
            <PopoverContent className="w-100 p-0" side="top">
                ...
                <div className="px-4 py-3 max-x-80 overflow-y-auto">
                    {error ? (
                        <div>
                        ...
                        <MessageResponse parseIncompleteMarkdown={status!=='ready'}>{composedText}</MessageResponse>
                ...
                disabled={!composedText}>Accept</Button>
                </div>
            </PopoverContent>
        </Popover>
    )
